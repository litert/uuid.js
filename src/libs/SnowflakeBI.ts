...
export type TEncoding = number | 'hex' | 'bin' | 'oct' | 'dec' | 'int64' | 'buffer' | 'bigint';
...
export interface IFactoryOptions<T extends TEncoding> {
...
export interface IOptions<T extends TEncoding> extends IFactoryOptions<T> {
...
export type TGeneratorOutput<T> = T extends 'int64' ? IUInt64 :
    T extends 'buffer' ? Buffer :
    T extends 'bigint' ? any :
    string;

export interface IGenerateorBasicInfo<T extends TEncoding> {
     ...
     */
    readonly defaultEncoding: T;
}

export interface IGenerateor<T extends TEncoding> extends IGenerateorBasicInfo<T> {

    (encoding?: T): TGeneratorOutput<T>;
...
export interface IFactory<T extends TEncoding = TDefaultEncoding> extends IGenerateorBasicInfo<T> {

    /**
     * Create a new Snowflake UUID generator.
     * @param options The options of generator.
     */
    create<TE extends TEncoding = T>(options?: IOptions<TE>): IGenerateor<TE>;
}
...
export function createFactory<T extends TEncoding = TDefaultEncoding>(
    options: IFactoryOptions<T> = DEFAULT_FACTORY_OPTIONS as any
): IFactory<T> {
